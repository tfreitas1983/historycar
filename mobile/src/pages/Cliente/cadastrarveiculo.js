...
export default function CadastrarVeiculos  ({ navigation }) {
 ...
 let modelos = [];
  ...
  let idMarca = null;//, setIdMarca] = useState('');
  ...
  let listamodelos = null;
  let mostrarmodelos = null;
  ...
  async function PegaModelos (item) {
    ...
    if (idMarca) {
      ...
      if (modelos) {
         ...
         listamodelos = modelos.modelos.map(itens => ( {             
           id: itens.codigo,
           title: itens.nome
         }));   
         console.log('listamodelos', listamodelos);
      } 
      setLoading(false)
    } else {
      idMarca = item.id;
    ...
    setTimeout(() => {
      if (listamodelos.length > 0) {
        ...
        renderItem={(items, text) => <Text style={{ color: '#fff', padding: 15 }}>{items.title}</Text>}
        ...
        mostrarmodelos = <Text>Sem resposta</Text>
      }  
    }, 5000);
 ...
 if (listamodelos) {
   mostrarmodelos = <SelectDropdown
   data={listamodelos.map(item => {return item.title})}
   defaultButtonText="Selecione o modelo"              
 ...
 />
   ...
   mostrarmodelos = <Text>Aguardando...</Text>
 }
